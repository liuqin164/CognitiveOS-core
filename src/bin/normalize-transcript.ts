...
import {
  normalizeAppPrivateMixedEventExport,
  normalizeDelimitedRecords,
  normalizeJsonArrayRecords,
  normalizeJsonlMixedEventLogExport,
  normalizeJsonlRecords,
  writeNormalizedConversationMarkdown,
  type ExportBridgeMarker,
...
export async function runNormalizeTranscript(argv: string[]): Promise<NormalizeTranscriptCliResult | void> {
  ...
  const sourceRefs = normalized.messages
    ...
    .filter((source): source is NormalizedMessageSource => Boolean(source));
  const result: NormalizeTranscriptCliResult = {
    inputPath,
    outputPath,
    title,
...
function normalizeInput(
  ...
  familyArg?: string,
  formatArg?: string,
): { family: NormalizationFamily; messages: NormalizedMessage[]; markers: ExportBridgeMarker[] } {
...
function resolveFamily(inputPath: string, familyArg?: string, formatArg?: string): NormalizationFamily {
  if (familyArg) return normalizeFamilyAlias(familyArg);
  if (formatArg) return normalizeFamilyAlias(formatArg);

  const ext = extname(inputPath).toLowerCase();
  if (ext === '.json') return 'json_array_transcript_export';
  if (ext === '.jsonl') return 'jsonl_transcript_export';
  if (ext === '.csv') return 'csv_transcript_export';
  if (ext === '.tsv') return 'tsv_transcript_export';
...
function normalizeFamilyAlias(value: string): NormalizationFamily {
  const normalized = value.trim().toLowerCase().replace(/_/g, '-');
  const aliases: Record<string, NormalizationFamily> = {
...
function printHumanSummary(result: NormalizeTranscriptCliResult): void {
