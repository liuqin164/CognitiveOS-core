...
function buildEnvLines(cfg: WizardConfig): string[] {
  const lines: string[] = [];

  const kv = (key: string, value: string | boolean) => {
  ...
  if (cfg.memoryBaseUrl) kv('AGENT_BRAIN_MODEL_MEMORY_BASE_URL', cfg.memoryBaseUrl);
  if (cfg.memoryApiKey)  kv('AGENT_BRAIN_MODEL_MEMORY_API_KEY',  cfg.memoryApiKey);
  ...
  if (cfg.reasoningBaseUrl) kv('AGENT_BRAIN_MODEL_REASONING_BASE_URL', cfg.reasoningBaseUrl);
  if (cfg.reasoningApiKey)  kv('AGENT_BRAIN_MODEL_REASONING_API_KEY',  cfg.reasoningApiKey);
...
function printUsageSnippet(cfg: WizardConfig, legacyEnv: boolean): void {
  console.log('');
...
async function main(): Promise<void> {
  ...
  if (args.yes) {
    ...
    if (!args.legacyEnv) ensureInstallDirs(target.homeDir);
