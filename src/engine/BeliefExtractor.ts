import type { BeliefCandidate, BeliefValidityKind, Neuron } from '../types/index.js';
...
interface SchemaRule {
  ...
  confidence: number;
  extractionReason: BeliefCandidate['extractionReason'];
  valueType?: BeliefCandidate['objectValue']['type'];
}

const PREFERENCE_AND_DECISION_RULES: SchemaRule[] = [
  {
...
const CONSTRAINT_RULES: SchemaRule[] = [
  {
...
const WORKFLOW_RULES: SchemaRule[] = [
  {
    name: 'workflow_cn',
...
export class BeliefExtractor {
  extract(context: ExtractionContext): BeliefCandidate[] {
    ...
    if (neuron.metadata.type !== 'chat' && neuron.metadata.type !== 'doc') return [];
  ...
  private extractSchemaRules(
    content: string,
    base: ReturnType<BeliefExtractor['getBaseContext']>,
    rules: SchemaRule[]
  ): BeliefCandidate[] {
    const matches: BeliefCandidate[] = [];
  ...
  private extractCondition(content: string): { raw: string; dsl: Record<string, unknown> } | undefined {
  ...
  private resolveValidityKind(
    temporalKind?: BeliefValidityKind,
    conditionalKind?: BeliefValidityKind
  ): BeliefValidityKind | undefined {
    if (conditionalKind) return conditionalKind;
    return temporalKind;
  }

  private getBaseContext(neuron: Neuron, sourceEventId: string, content: string) {
    return {
      neuronId: neuron.id,
      projectId: neuron.metadata.projectId,
      scope: neuron.metadata.projectId ? 'project' as const : 'global' as const,
  ...
  private normalizeEntity(raw: string): string | null {
    const entity = raw.trim().toLowerCase().replace(/^[\s"'`]+|[\s"'`?!.。，！？]+$/g, '');
    return entity.length >= 2 ? entity : null;
  }

  private normalizeFactValue(raw: string): string | null {
    const value = raw.trim().replace(/^[\s"'`]+|[\s"'`。！？,，]+$/g, '');
    return value.length >= 2 ? value : null;
  }

  private parseSequenceSteps(content: string): string[] {
  ...
  private buildConditionDsl(raw: string, lang: 'zh' | 'en'): Record<string, unknown> {
  ...
  private toConditionClause(part: string, lang: 'zh' | 'en'): Record<string, unknown> {
    if (lang === 'zh') {
  ...
  private flattenConditionClauses(expr: Record<string, unknown>): Record<string, unknown>[] {
    const kind = String(expr.kind || '');
    if (kind !== 'group') return [expr];

    const clauses = Array.isArray(expr.clauses) ? expr.clauses as Record<string, unknown>[] : [];
    return clauses.flatMap((clause) => this.flattenConditionClauses(clause));
  }

  private normalizeConditionOperators(raw: string, lang: 'zh' | 'en'): string {
  ...
  private stripOuterParentheses(raw: string): string {
    let text = raw.trim();
  ...
  private isBalancedParentheses(raw: string): boolean {
    let depth = 0;
    for (const ch of raw) {
      if (ch === '(') depth += 1;
      if (ch === ')') {
  ...
  private splitTopLevelByOperators(raw: string, operators: string[]): string[] {
    const parts: string[] = [];
    let buffer = '';
    let depth = 0;
    let index = 0;
  ...
  private parseNestedFactGraph(content: string): { path: string[]; nodes: string[]; edges: Array<{ from: string; to: string }> } | null {
  ...
  private parseExecutablePlanGraph(content: string): {
    entrySteps: string[];
    nodes: string[];
    edges: Array<{ from: string; to: string; kind: 'dependency' | 'failure' | 'recovery' | 'retry' | 'merge' | 'branch' | 'state_transition' }>;
    handlers: Array<{ trigger: string; on: 'failure'; steps: string[] }>;
    retryPolicies: Array<{ target: string; maxAttempts: number; backoff?: string }>;
    mergePoints: Array<{ into: string; branches: string[] }>;
    branchPoints: Array<{ from: string; branches: string[] }>;
    stateTransitions: Array<{ entity: string; from: string; to: string }>;
    policyExecutors: Array<{ policy: string; target: string; config?: string }>;
    executionGuards: Array<{ target: string; requires: string[] }>;
    stateMachines: Array<{ entity: string; states: string[] }>;
    policyRuntime: Array<{ policy: string; mode?: string; target: string }>;
    mergeConstraints: Array<{ into: string; requires: string[] }>;
    runtimeValidation: Array<{ target: string; checks: string[] }>;
    executorBindings: Array<{ target: string; executor: string; mode?: string }>;
    mergePropagation: Array<{ into: string; propagates: string[] }>;
  } | null {
    if (!/(plan|计划|rollback|recover|recovery|redeploy|retry|失败)/i.test(content)) return null;
    ...
    if (entrySteps.length < 2 && handlers.length === 0 && retryPolicies.length === 0 && mergePoints.length === 0 && branchPoints.length === 0 && stateTransitions.length === 0 && policyExecutors.length === 0 && executionGuards.length === 0 && stateMachines.length === 0 && policyRuntime.length === 0 && mergeConstraints.length === 0 && runtimeValidation.length === 0 && executorBindings.length === 0 && mergePropagation.length === 0) return null;

    const nodes = new Set<string>(entrySteps);
    const edges: Array<{ from: string; to: string; kind: 'dependency' | 'failure' | 'recovery' | 'retry' | 'merge' | 'branch' | 'state_transition' }> = [];
    ...
    for (const handler of handlers) {
      ...
      for (const step of handler.steps) nodes.add(step);
    ...
    for (const guard of executionGuards) {
      ...
      for (const requirement of guard.requires) nodes.add(requirement);
    }

    for (const machine of stateMachines) {
      nodes.add(machine.entity);
      for (const state of machine.states) nodes.add(state);
    }
    ...
    for (const constraint of mergeConstraints) {
      ...
      for (const requirement of constraint.requires) nodes.add(requirement);
    }

    for (const validation of runtimeValidation) {
      nodes.add(validation.target);
      for (const check of validation.checks) nodes.add(check);
    }
    ...
    for (const propagation of mergePropagation) {
      ...
      for (const item of propagation.propagates) nodes.add(item);
    }
  ...
  private parsePlanHandlers(content: string): Array<{ trigger: string; on: 'failure'; steps: string[] }> {
    const handlers: Array<{ trigger: string; on: 'failure'; steps: string[] }> = [];
    ...
    while ((match = cnPattern.exec(content)) !== null) {
      ...
      if (trigger && steps.length > 0) handlers.push({ trigger, on: 'failure', steps });
    }
  ...
  private parseHandlerSteps(raw: string): string[] {
  ...
  private parseRetryPolicies(content: string): Array<{ target: string; maxAttempts: number; backoff?: string }> {
    const policies: Array<{ target: string; maxAttempts: number; backoff?: string }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (target && maxAttempts > 0) policies.push({ target, maxAttempts, backoff });
    }
  ...
  private parseMergePoints(content: string): Array<{ into: string; branches: string[] }> {
    const mergePoints: Array<{ into: string; branches: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (left && right && into) mergePoints.push({ into, branches: [left, right] });
    }
  ...
  private parseBranchPoints(content: string): Array<{ from: string; branches: string[] }> {
    const branchPoints: Array<{ from: string; branches: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (from && left && right) branchPoints.push({ from, branches: [left, right] });
    }
  ...
  private parseStateTransitions(content: string): Array<{ entity: string; from: string; to: string }> {
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (entity && from && to) transitions.push({ entity, from, to });
    }
  ...
  private parsePolicyExecutors(content: string): Array<{ policy: string; target: string; config?: string }> {
    const executors: Array<{ policy: string; target: string; config?: string }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (policy && target) executors.push({ policy, target, config });
    }
  ...
  private parseExecutionGuards(content: string): Array<{ target: string; requires: string[] }> {
    const guards: Array<{ target: string; requires: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (target && requirement) guards.push({ target, requires: [requirement] });
    }
  ...
  private parseStateMachines(content: string): Array<{ entity: string; states: string[] }> {
    const machines: Array<{ entity: string; states: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (entity && states.length >= 2) machines.push({ entity, states });
    }
  ...
  private parsePolicyRuntime(content: string): Array<{ policy: string; mode?: string; target: string }> {
    const runtimes: Array<{ policy: string; mode?: string; target: string }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (policy && target) runtimes.push({ policy, mode: mode || undefined, target });
    }
  ...
  private parseMergeConstraints(content: string): Array<{ into: string; requires: string[] }> {
    const constraints: Array<{ into: string; requires: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      const left = this.normalizeEntity(match[2] || '');
      const right = this.normalizeEntity(match[3] || '');
      if (into && left && right) constraints.push({ into, requires: [left, right] });
    }
  ...
  private parseRuntimeValidation(content: string): Array<{ target: string; checks: string[] }> {
    const validations: Array<{ target: string; checks: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      const left = this.normalizeEntity(match[2] || '');
      const right = this.normalizeEntity(match[3] || '');
      if (target && left && right) validations.push({ target, checks: [left, right] });
    }
  ...
  private parseExecutorBindings(content: string): Array<{ target: string; executor: string; mode?: string }> {
    const bindings: Array<{ target: string; executor: string; mode?: string }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      if (target && executor) bindings.push({ target, executor, mode });
    }
  ...
  private parseMergePropagation(content: string): Array<{ into: string; propagates: string[] }> {
    const propagations: Array<{ into: string; propagates: string[] }> = [];
    ...
    while ((match = enPattern.exec(content)) !== null) {
      ...
      const left = this.normalizeEntity(match[2] || '');
      const right = this.normalizeEntity(match[3] || '');
      if (into && left && right) propagations.push({ into, propagates: [left, right] });
    }
  ...
  private dedupeCandidates(candidates: BeliefCandidate[]): BeliefCandidate[] {
    const deduped = new Map<string, BeliefCandidate>();
