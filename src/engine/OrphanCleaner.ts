...
export class OrphanCleaner {
  constructor(
    ...
    private readonly options: OrphanCleanerOptions = {}
  ) {}

  async run(projectId: string): Promise<{ orphansMarked: number }> {
    ...
    const candidates = this.memoryGraph.getAllNeurons()
      ...
      .slice(0, batchSize);
    for (const neuron of candidates) this.memoryGraph.updateNeuronMetadata(neuron.id, { status: 'suspect' });
    return { orphansMarked: candidates.length };
  }

  private degree(neuronId: string): number {
