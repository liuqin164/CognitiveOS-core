...
import {
  ...
  extractTemporalHints,
  inferEntityTypeFromText,
  isOwnershipSignal,
...
export class LocalSemanticCompiler {
  ...
  private compile(sourceType: 'memory' | 'query', text: string): SemanticCompilation {
    const normalized = normalizeLexiconText(text);
    const lowered = normalized.toLowerCase();
    const entities: SemanticEntityCandidate[] = [];
    const topics: SemanticTopicLink[] = [];
    const temporalHints: Array<QueryIR['temporal']['relative']> = [];
    ...
    const pushEntity = (value: string | undefined, type: string | undefined, confidence: number): void => {
      if (!value) return;
      const textValue = value.trim();
      if (!textValue) return;
      if (entities.some((entity) => entity.text === textValue && entity.type === type)) return;
    ...
    if (issueFamilies.length > 0) {
      ...
      for (const issue of issueMatches) issueHints.push(issue.trim());
    ...
    if (relativeRefs.length > 0) {
      ...
      for (const ref of relativeRefs) relativeReferences.push(ref.trim());
    }
