...
type FileChunkEvidenceRow = FileChunkRow & {
  file_path: string;
...
export class FileChunkStore {
  ...
  replaceChunks(assetId: string, chunks: InsertChunkInput[]): FileChunkRecord[] {
    ...
    const records = chunks.map((chunk) => {
      ...
      stmt.run(
        ...
        record.neuronId,
        record.chunkIndex,
        record.blockStartIndex,
        record.blockEndIndex,
        record.kind,
        record.tokenEstimate || null,
        record.textHash,
        record.createdAt,
  ...
  listByAsset(assetId: string): FileChunkRecord[] {
    ...
    `).all(assetId) as FileChunkRow[]).map((row) => this.mapRow(row));
  }

  listContext(assetId: string, chunkIndex: number, radius: number = 1): FileChunkEvidence[] {
    ...
    `).all(assetId, start, end) as FileChunkEvidenceRow[]).map((row) => this.mapEvidenceRow(row));
  }
  ...
  listEvidenceByNeuronIds(neuronIds: string[]): FileChunkEvidence[] {
    ...
    `).all(...neuronIds) as FileChunkEvidenceRow[]).map((row) => this.mapEvidenceRow(row));
  }

  groupEvidenceByAsset(evidence: FileChunkEvidence[]): FileEvidence[] {
    const grouped = new Map<string, FileEvidence>();
  ...
  private insertSequentialEdges(chunks: FileChunkRecord[]): void {
  ...
  private mapRow(row: FileChunkRow): FileChunkRecord {
    return {
  ...
  private mapEvidenceRow(row: FileChunkEvidenceRow): FileChunkEvidence {
    return {
      ...this.mapRow(row),
      text: row.content,
