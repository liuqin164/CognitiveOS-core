...
export class ResonanceCore {
  private static chainStore: IReasoningChainStore | null = null;
  ...
  /** 能量传导（含推理链强化 + 置信度乘数） */
  static async propagateEnergy(
    anchorNeuron: Neuron,
    getNeuron: (id: string) => Neuron | null,
    maxHops: number = 5
  ): Promise<Map<string, number>> {
    const energyMap = new Map<string, number>();
    const queue: Array<{ neuronId: string; energy: number; hops: number }> = [
    ...
    while (queue.length > 0) {
      ...
      if (visited.has(neuronId) || hops > maxHops) continue;
  ...
  /** 获取衰减系数（推理链强化） */
  private static getDecayFactor(synapse: Synapse, sourceId: string): number {
  ...
  /** 突触类型衰减系数 */
  private static getSynapseDecay(type: string): number {
    const DECAY = {
      'Caused_by': 0.9,
      'Sequence': 0.6,
      'Similar': 0.75,
    ...
    return (DECAY as any)[type] || DECAY.Default;
  }

  private static calculateDecayedEnergy(initialEnergy: number, hops: number): number {
    return initialEnergy * Math.pow(0.8, hops);
  }

  /** 应用能量排序（含置信度） */
  static applyEnergyRanking(
    candidates: Neuron[],
    energyMap: Map<string, number>,
    temporalOp?: TemporalOperator,
    spatialOp?: SpatialOperator
  ): Array<{ neuron: Neuron; energy: number }> {
    ...
    return filtered.map(neuron => {
      ...
      if (temporalOp) energy += this.calculateTemporalEnergy(neuron, temporalOp);
      if (spatialOp) energy += this.calculateSpatialEnergy(neuron, spatialOp);

      return { neuron, energy };
    }).sort((a, b) => b.energy - a.energy);
  }

  private static calculateTemporalEnergy(neuron: Neuron, op: TemporalOperator): number {
  ...
  private static calculateSpatialEnergy(neuron: Neuron, op: SpatialOperator): number {
