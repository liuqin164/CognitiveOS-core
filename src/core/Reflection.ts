...
import type { Neuron, Synapse, MemoryAnchor } from '../types/index.js';
...
export class Reflection {
  ...
  private activationLog = new Map<string, number[]>();
  private coOccurrenceMap = new Map<string, Map<string, number>>();
  ...
  /** 每日权重更新 */
  async dailyWeightUpdate(): Promise<void> {
  ...
  /** 查找冲突的旧记忆（向量检索 + 极性碰撞） */
  findConflictingNeurons(
    newNeuron: Neuron,
    vectorSearchFn?: (vector: number[], k: number) => Array<{ id: string; score: number }>
  ): Neuron[] {
    const conflicting: Neuron[] = [];
  ...
  /** 极性碰撞检测 */
  private checkPolarityCollision(a: Neuron, b: Neuron): boolean {
  ...
  private async findCoOccurrences(): Promise<Map<string, number>> {
  ...
  private recordCoOccurrence(neuronId: string): void {
  ...
  private boostSynapses(neuronId: string): void {
    const synapses = this.memoryGraph.getSynapses(neuronId);
    for (const synapse of synapses) {
  ...
  private getSynapse(sourceId: string, targetId: string): Synapse | null {
    return this.memoryGraph.getSynapses(sourceId).find(s => s.targetId === targetId) || null;
  }

  private updateSynapse(sourceId: string, synapse: Synapse): void {
    this.memoryGraph.addSynapse(sourceId, synapse);
  }

  async createAnchor(neuronIds: string[], projectId?: string): Promise<MemoryAnchor> {
    return this.memoryGraph.createAnchor(neuronIds, projectId);
  }

  getActivationStats() {
    let total = 0;
    const top: Array<{ id: string; count: number }> = [];
  ...
  cleanupOldActivations(maxAge: number = 86400000): void {
    const now = Date.now();
    for (const [id, acts] of this.activationLog) {
      const recent = acts.filter(t => now - t < maxAge);
      if (recent.length === 0) this.activationLog.delete(id);
      else this.activationLog.set(id, recent);
    }
  }
}