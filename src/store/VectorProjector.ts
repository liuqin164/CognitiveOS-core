...
export class VectorProjector {
  constructor(
    ...
    private projectionName: string = 'hnsw_main'
  ) {}
  ...
  async fullRebuild(reason: string): Promise<void> {
    ...
    await this.memoryGraph.forEachNeuronVectorPage(
      pageSize,
      async (rows) => {
        pageNo += 1;
  ...
  private applyEvent(event: MemoryEvent): void {
    switch (event.eventType) {
      ...
      case 'RESTORED': {
        ...
        if (!neuron) return;
        if (neuron.metadata.status === 'archived') return;
        if (!neuron.coordinates.V || neuron.coordinates.V.length === 0) return;
