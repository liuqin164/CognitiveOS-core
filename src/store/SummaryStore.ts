...
type SummaryRow = {
  ...
  deep_write_run_id: string | null;
  deep_write_candidate_id: string | null;
  created_at: number;
  updated_at: number;
...
export class SummaryStore {
  ...
  migrateLegacyFactSummaries(): number {
    this.initSchema();
    const factColumns = this.db.prepare(`PRAGMA table_info(facts)`).all() as Array<{ name: string }>;
    if (!factColumns.some((column) => column.name === 'predicate_family')) return 0;
  ...
  insertSummary(input: SummaryInput): SummaryRecord {
    ...
    `).run(
      ...
      record.deepWriteRunId || null,
      record.deepWriteCandidateId || null,
      record.createdAt,
      record.updatedAt
    );
    return record;
  ...
  getById(id: string): SummaryRecord | null {
    const row = this.db.prepare(`SELECT * FROM deep_write_summaries WHERE summary_id = ?`).get(id) as SummaryRow | null;
  ...
  findRelevant(query: string, projectId?: string, limit: number = 3): SummaryRecord[] {
  ...
  private fallbackFindRelevant(query: string, projectId: string | undefined, limit: number): SummaryRecord[] {
