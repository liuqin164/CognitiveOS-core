...
export function parseVectorDimensionValue(
  value: unknown,
  fieldName: string,
  ...
  if (typeof value === 'number') {
    ...
    if (/^[0-9]+$/.test(trimmed)) parsed = Number(trimmed);
  }
...
export function addVectorDimensionDiagnostics(
  dimension: number,
  diagnostics: ConfigDiagnosticLike[],
): void {
...
export function vectorDimensionWarningMessage(dimension: number): string {
  return [
...
export function estimateVectorBytes(
  dimension: number,
  count = VECTOR_DIMENSION_ESTIMATE_COUNT,
): number {
