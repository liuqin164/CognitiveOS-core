...
export interface AsyncEnrichmentTrigger {
  kind:
    | 'ingest_compiled_fact_count_zero'
    | 'self_correction_pattern'
    | 'multi_candidate_single_fact'
    | 'unseen_entity_binding_suspect'
    | 'low_compiler_confidence';
  detail: string;
}
...
export interface AsyncEnrichmentHook {
  enrich(
    input: AsyncEnrichmentHookInput,
    context: AsyncEnrichmentWritebackContext
  ): Promise<AsyncEnrichmentWriteback | void> | AsyncEnrichmentWriteback | void;
}

export class NoopAsyncEnrichmentHook implements AsyncEnrichmentHook {
  enrich(): void {}
}

export function createAsyncEnrichmentRunId(): string {
