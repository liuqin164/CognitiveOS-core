...
export interface ILLMClarifier {
  clarify(prompt: string, evidence: BrainRecallResult): Promise<{ finalAnswer: string } | string>;
}
...
export interface ISkillMemoryStore {
  ...
  listAll(): Neuron[];
  getExecutionStatsForTags?(
    tags: string[],
    options?: { projectId?: string; before?: number; after?: number }
  ): Record<string, { samples: number; successRate: number }>;
}
...
export interface IAuditLedger {
  ...
  predictionStats?(intentTag: string): { accuracy: number; verifiedCount?: number; total?: number; verified?: number };
}
...
export interface RuntimeSelfManifestLike {
  ...
  models: { roles: unknown[] };
  fileAssets: { indexedAssetCount: number };
