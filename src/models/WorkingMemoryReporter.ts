...
export class WorkingMemoryReporter {
  constructor(private readonly userModelStore: UserModelStore) {}
  ...
  formatDelta(delta: MemoryDelta): string {
    const lines: string[] = [];
    if (delta.newInsights.length) lines.push(`新出现：${delta.newInsights.map(formatInsight).join('；')}`);
    if (delta.strengthenedInsights.length) lines.push(`增强：${delta.strengthenedInsights.map(formatTrend).join('；')}`);
    if (delta.weakenedInsights.length) lines.push(`减弱：${delta.weakenedInsights.map(formatTrend).join('；')}`);
    if (delta.expiredInsights.length) lines.push(`过期：${delta.expiredInsights.map((insight) => insight.content).join('；')}`);
    return lines.length ? ['[近期变化]', ...lines].join('\n') : '';
  }
}

function insightKey(insight: UserInsight): string {
  return `${insight.projectId}|${insight.category}|${insight.content}`;
}

function formatInsight(insight: UserInsight): string {
  return `${insight.content}（置信度 ${insight.confidence.toFixed(1)}）`;
}

function formatTrend(insight: UserInsight): string {
