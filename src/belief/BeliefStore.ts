...
export class BeliefStore {
  ...
  private static readonly SCOPE_PRIORITY: Record<ScopeType, number> = {
    file: 5,
    session: 4,
    project: 3,
    agent: 2,
  ...
  getBeliefHistoryForCanonicalKeys(
    ...
    for (const row of rows) {
      ...
      if (bucket.length >= (options.limitPerCanonical ?? 6)) continue;
      bucket.push(belief);
  ...
  getExecutionFeedbackNeuronSignals(records: PolicyExecutionRecord[]): Array<{
    ...
    for (const row of rows) {
      const belief = this.mapBelief(row);
      if (!belief.sourceNeuronId) continue;
      ...
      current.latestUpdatedAt = Math.max(
        current.latestUpdatedAt || 0,
        feedback.latestUpdatedAt || 0
      ) || undefined;
  ...
  applyExecutionFeedbackCalibration(records: PolicyExecutionRecord[], now: number = Date.now()): number {
    ...
    for (const row of rows) {
      ...
      const nextTrust = this.clamp(
        belief.trustScore + feedback.executed * 0.02 - feedback.failed * 0.03,
        0.1,
        1.0
      );
      const nextUpdatedAt = Math.max(belief.updatedAt, feedback.latestUpdatedAt || belief.updatedAt);
      if (Math.abs(nextTrust - belief.trustScore) < 0.0001 && nextUpdatedAt === belief.updatedAt) continue;
  ...
  getEvidenceNeuronIds(beliefIds: string[], limitPerBelief: number = 3): string[] {
    ...
    `).all(...beliefIds) as Array<{ belief_id: string; neuron_id: string | null }>;

    const limited = new Map<string, string[]>();
    for (const row of rows) {
      if (!row.neuron_id) continue;
      const bucket = limited.get(row.belief_id) || [];
      if (bucket.length >= limitPerBelief) continue;
      if (!bucket.includes(row.neuron_id)) bucket.push(row.neuron_id);
  ...
  upsert(candidate: BeliefCandidate, now: number = Date.now()): {
    ...
    this.db.transaction(() => {
      ...
      `).run(
        ...
        belief.subject,
        belief.predicate,
        belief.objectValue.normalized || belief.objectValue.raw,
        belief.objectValue.type,
        belief.canonicalKey,
        belief.confidence,
        belief.trustScore,
        belief.sourceNeuronId || null,
        belief.sourceEventId || null,
        belief.sourceType,
        belief.validityKind,
        belief.validFrom,
        belief.validTo || null,
        belief.supersedesBeliefId || null,
        null,
        belief.contradictionGroup || null,
        belief.status,
        belief.explanation || null,
        belief.metadata ? JSON.stringify(belief.metadata) : null,
        belief.createdAt,
        belief.updatedAt
      );
  ...
  resolveConflict(
    ...
    conflicts: BeliefConflictCandidate[],
    now: number = Date.now()
  ): BeliefRevisionDecision {
    if (conflicts.length === 0) return { action: 'insert' };

    const activeConflicts = conflicts
      .map((conflict) => conflict.existing)
      .filter((belief) => belief.status === 'active' || belief.status === 'suspect');

    if (activeConflicts.length === 0) return { action: 'insert' };
    ...
    for (const existing of activeConflicts) {
      ...
      if (compare === 'incoming') supersedeIds.push(existing.id);
      else if (compare === 'existing') strongerExistingCount += 1;
    }
  ...
  private compareBeliefs(incoming: BeliefCandidate, existing: BeliefRecord, now: number): 'incoming' | 'existing' | 'tie' {
    const incomingStrength = this.computeBeliefStrength({
      confidence: incoming.confidence,
      trustScore: incoming.trustScore ?? this.getSourceTrust(incoming.sourceType),
      sourceType: incoming.sourceType,
    ...
    const newerPreferenceRevision = (
      (incoming.predicate.startsWith('preference.') || incoming.predicate.startsWith('decision.')) &&
      incomingValidFrom > existing.validFrom &&
      sameSourceFamily &&
      delta > -0.05
    );

    if (newerPreferenceRevision) return 'incoming';
    if (delta >= 0.12) return 'incoming';
    if (delta <= -0.12) return 'existing';
    return 'tie';
  }

  private computeBeliefStrength(input: {
    confidence: number;
    trustScore: number;
  ...
  private resolveWeightMatrix(predicate: string, sourceType: SourceType): {
    confidenceW: number;
    trustW: number;
    freshnessW: number;
    scopeW: number;
  ...
  private computeTemporalFreshness(ageHours: number, predicate: string, sourceType: SourceType): number {
    let halfLifeHours = 24 * 30;
    if (predicate.startsWith('preference.')) halfLifeHours = 24 * 10;
    else if (predicate.startsWith('decision.')) halfLifeHours = 24 * 14;
    else if (predicate.startsWith('constraint.')) halfLifeHours = 24 * 21;
    else if (sourceType === 'verified_fact') halfLifeHours = 24 * 90;
    return Math.exp(-Math.log(2) * Math.max(ageHours, 0) / halfLifeHours);
  }

  private computeScopeWeight(scope: ScopeType, predicate: string): number {
    const base = {
      global: 0.45,
      agent: 0.5,
      project: 0.68,
    ...
    if (predicate.startsWith('fact.') && scope === 'global') return 0.8;
    if (predicate.startsWith('fact.') && scope === 'file') return 0.6;
    return base;
  }

  private buildContradictionGroup(candidate: Pick<BeliefCandidate, 'subject' | 'predicate' | 'scope'>): string {
  ...
  private isSameBeliefValue(a: BeliefValue, b: BeliefValue): boolean {
    return (a.normalized || a.raw) === (b.normalized || b.raw);
  }

  private toCanonicalKey(subject: string, predicate: string, scope: ScopeType): string {
    return `${subject}|${predicate}|${scope}`;
  }

  private getSourceTrust(sourceType: SourceType): number {
  ...
  private scoreBeliefForQuery(
    ...
    const planDsl = belief.metadata?.planDsl as Record<string, unknown> | undefined;
    if (planDsl && input.structuredTargets.predicatePrefix === 'workflow.plan.') {
      ...
      for (const hint of input.semantics?.entityHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.7;
      }
      for (const hint of input.semantics?.environmentHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.55;
      }
      for (const hint of input.semantics?.stateHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.75;
      }
      for (const hint of input.semantics?.policyHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.75;
      }
      for (const hint of input.semantics?.guardHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.65;
      }
      ...
      for (const hint of input.semantics?.valueHints || []) {
        if (planText.includes(hint.toLowerCase())) score += 0.6;
      }
      ...
      if (executionAnalysis.blockedSteps.length > 0) score -= Math.min(executionAnalysis.blockedSteps.length, 3) * 0.18;
    ...
    if (input.projectId && belief.projectId === input.projectId) score += 0.8;
    if (belief.scope === 'project') score += 0.3;
    if (belief.scope === 'global') score += 0.1;
    score += belief.confidence * 0.6;
    score += belief.trustScore * 0.6;

    if (belief.status === 'active') score += 0.4;
    return score;
  }

  private computeExecutionFeedbackForBelief(
    belief: BeliefRecord,
    records: PolicyExecutionRecord[]
  ): {
    matchedExecutions: number;
    executed: number;
    failed: number;
    ...
    const policies = Array.from(new Set(
      [...serialized.matchAll(/"(?:policy|name)"\s*:\s*"([^"]+)"/g)].map((match) => match[1] || '')
    )).filter(Boolean);

    const matched = records.filter((record) => {
      if (policyGroup && record.policyGroup === policyGroup) return true;
      if (policies.includes(record.policy)) return true;
      return false;
    });
  ...
  private extractStructuredTargets(
    query: string,
    intent: 'fact_lookup' | 'preference_lookup' | 'decision_lookup' | 'constraint_lookup' | 'trace' | 'debug_context' | 'recall' | undefined,
    tokens: string[],
    semantics?: {
      subjectHint?: string;
      predicateHint?: 'preference' | 'decision' | 'constraint' | 'fact' | 'workflow' | 'graph' | 'sequence' | 'plan';
    ...
    if (intent === 'preference_lookup' || predicateHint === 'preference') {
      ...
      if (/(喜欢|prefer|like)/i.test(query)) target.expectedValue = 'like';
      if (/(讨厌|dislike|不喜欢)/i.test(query)) target.expectedValue = 'dislike';
    ...
    if (!target.expectedValue && semantics?.valueHints?.[0]) target.expectedValue = semantics.valueHints[0];
    return target;
  }
