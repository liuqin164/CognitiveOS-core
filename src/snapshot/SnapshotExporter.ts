...
import type { SnapshotHeader, SnapshotMeta } from './SnapshotHeader.js';
...
export class SnapshotExporter {
  constructor(private readonly options: SnapshotExporterOptions) {}

  async export(dbPath: string, outputPath: string): Promise<SnapshotMeta> {
    if (dbPath === ':memory:') {
  ...
  private readSchemaVersion(db: Database): number {
    try {
  ...
  private readNeuronCount(db: Database): number {
    try {
      const row = db.prepare(`SELECT COUNT(*) AS count FROM neurons WHERE is_deleted = 0`).get() as { count?: number } | undefined;
