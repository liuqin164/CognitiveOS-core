...
const ALLOWED_ACTIONS: Set<string> = new Set<BrainToolName>([
  'brain_recall',
  'get_neuron_context',
  'expand_entity',
  'find_file_assets',
 ...
 */
export function parse(llmOutput: string): BrainToolCall | null {
  if (!llmOutput || typeof llmOutput !== 'string') return null;

  const trimmed = llmOutput.trim();
  if (trimmed.startsWith('[')) {
    try {
      if (Array.isArray(JSON.parse(trimmed))) return null;
    } catch {
  ...
  const toolName = action as BrainToolName;
  ...
  const call: BrainToolCall = { action: toolName };

  if (typeof obj['query'] === 'string')        call.query        = obj['query'];
  if (typeof obj['entity_hint'] === 'string')  call.entity_hint  = obj['entity_hint'];
  if (typeof obj['limit'] === 'number')        call.limit        = obj['limit'];
  if (typeof obj['neuron_id'] === 'string')    call.neuron_id    = obj['neuron_id'];
  if (typeof obj['entity_name'] === 'string')  call.entity_name  = obj['entity_name'];
  if (typeof obj['entity_type'] === 'string')  call.entity_type  = obj['entity_type'];
  if (typeof obj['extension'] === 'string')    call.extension    = obj['extension'];
  if (typeof obj['mime_type'] === 'string')    call.mime_type    = obj['mime_type'];
  if (typeof obj['asset_id'] === 'string')     call.asset_id     = obj['asset_id'];
  if (typeof obj['chunk_index'] === 'number')  call.chunk_index  = obj['chunk_index'];
  if (typeof obj['radius'] === 'number')       call.radius       = obj['radius'];
  if (typeof obj['reason'] === 'string')       call.reason       = obj['reason'];
 ...
 */
export function isFinalAnswer(llmOutput: string): boolean {
