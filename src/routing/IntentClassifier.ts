...
const TEMPORAL_RECALL_PATTERN =
  /last (?:week|month|day|time)|yesterday|ago|previously|before|earlier|上次|之前|上周|上个月/i;
const CORRECTION_CHECK_PATTERN =
  /change.*mind|update|correct|fix|wrong|actually|no wait|wait no|改|纠正|更新/i;
...
const CROSS_DOMAIN_TOPIC_GROUPS: Record<string, RegExp> = {
...
export class IntentClassifier {
  classify(query: string, options: { projectId?: string } = {}): {
  ...
  private isCrossDomainQuery(query: string, projectId?: string): boolean {
    ...
    return (
      distinctGroups.has('connectivity') &&
      /\b(thing|things|stuff|history|dealing with|dealt with)\b|所有|都有哪些/i.test(query)
    );
  }

  private extractTemporalHint(query: string): string | undefined {
    const match = query.match(
      /last (?:week|month|day|time)|yesterday|\d+\s+(?:days?|weeks?|months?|years?)\s+ago|previously|before|earlier|上次|之前|上周|上个月/i
    );
    return match?.[0];
  }

  private extractEntityHint(query: string): string | undefined {
    const candidates: Array<{ index: number; value: string }> = [];
    const pushMatches = (pattern: RegExp, groupIndex = 1) => {
