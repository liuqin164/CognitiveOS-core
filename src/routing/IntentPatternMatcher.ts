...
export const SYSTEM_INTENT_PRIORITY: readonly SystemIntent[] = [
...
export const SYSTEM_INTENT_PATTERNS: Readonly<Record<SystemIntent, readonly RegExp[]>> = {
  'system_query.tasks': [
    /现在.*(有什么|哪些).*(任务|工作)/,
    /任务.*(状态|列表|进度|有哪些)/,
    /有.*(哪些|什么).*(任务|工作).*(?:在)?运行/,
...
export class IntentPatternMatcher {
  readonly patterns = SYSTEM_INTENT_PATTERNS;
