...
const APPROVE_SUBJECT_PATTERNS: RegExp[] = [
...
const REJECT_SUBJECT_PATTERNS: RegExp[] = [
...
const SUBJECT_STRIP_PATTERN =
  /^(?:that|it|the|这个|那个|本次|这次|该)\s+|[\s，。,.:：;；!?！]+$/g;
...
export class ConfirmationPhraseMatcher {
  ...
  private extractSubject(message: string, patterns: readonly RegExp[]): string | null {
