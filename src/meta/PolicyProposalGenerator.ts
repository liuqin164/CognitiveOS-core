...
import type { ObservationPattern, PolicyProposal, ProposalCategory, ProposalEvidence } from './types.js';

const DEFAULT_EVAL_PLAN: EvalSuiteName[] = ['memory_recall'];
const PATCH_ONLY_CATEGORIES = new Set<ProposalCategory>([
  'risk_rule',
  'observation_filter',
...
export class PolicyProposalGenerator {
  ...
  private buildEvidence(pattern: ObservationPattern): ProposalEvidence[] {
  ...
  private buildBase(pattern: ObservationPattern): ProposalBase | null {
    switch (pattern.type) {
  ...
  private finalizeProposal(proposal: PolicyProposal): PolicyProposal {
    return {
      ...proposal,
