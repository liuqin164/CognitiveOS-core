import { EN_ENTITY_TYPE_TERMS, EN_ISSUE_KEYWORDS, EN_ISSUE_QUALIFIERS, EN_LONG_TERM_SIGNAL_TERMS, EN_PENDING_PATTERNS, EN_REFERENCE_SIGNALS, EN_RELATION_PHRASES, EN_SHORT_REPLY_BINDING } from './language-packs/en.js';
import { ZH_ENTITY_TYPE_TERMS, ZH_ISSUE_KEYWORDS, ZH_ISSUE_QUALIFIERS, ZH_LONG_TERM_SIGNAL_TERMS, ZH_PENDING_PATTERNS, ZH_REFERENCE_SIGNALS, ZH_RELATION_PHRASES, ZH_SHORT_REPLY_BINDING } from './language-packs/zh.js';
export const ENTITY_TYPE_LEXICON = {
...
export const REFERENCE_SIGNAL_LEXICON = {
...
export const ISSUE_FAMILY_LEXICON = {
    connectivity_issue: {
...
function collapseSurfaceWhitespace(value) {
    return value.trim().replace(/\s+/g, ' ');
}
function stripRepeatedPrefixes(value, patterns) {
...
function escapeLexiconRegex(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function cleanDeviceSurfaceCandidate(candidate, fullText) {
    ...
    if (!stripped)
        return undefined;
    if (/\bproject\b|项目/i.test(stripped))
        return undefined;
    if (/^(?:耳机|键盘|鼠标|显示器|bluetooth\s+(?:earbuds?|earphones?)|earbuds?|earphones?|keyboard|mouse|monitor|device|headset)$/i.test(stripped)) {
        if (fullText) {
            ...
            if (new RegExp(`(?:the\\s+previous|previous|the\\s+new|new|the\\s+old|old|this|that)\\s+${escaped}\\b`, 'i').test(fullText)
...
function cleanProjectSurfaceCandidate(candidate) {
    const stripped = stripRepeatedPrefixes(candidate, [
    ...
    if (!stripped)
        return undefined;
    if (/^(?:项目|project|sdk|library|platform|api|repository|repo)$/i.test(stripped))
        return undefined;
    return stripped;
}
export function normalizeLexiconText(text) {
    return text
        .trim()
        .replace(/蓝芽/g, '蓝牙')
        .replace(/藍牙/g, '蓝牙')
...
export function includesLexiconTerm(text, terms) {
    return terms.some((term) => text.toLowerCase().includes(term.toLowerCase()));
}
function normalizeShortReplyText(text) {
...
export function inferEntityTypeFromText(text) {
    const lowered = text.toLowerCase();
    const orderedTypes = ['device', 'project', 'brand', 'issue', 'person'];
...
export function extractRelativeReferences(text) {
    ...
    const patternRefs = Array.from(normalizeLexiconText(text).matchAll(/((?:之前那个|前一个|上一个|新的那个|新的项目|新买的|新项目|旧的那个|旧的|那个|这个|它)(?:[\u4e00-\u9fa5A-Za-z0-9._ -]{0,6}(?:耳机|键盘|鼠标|显示器|项目|device|project)|(?:耳机|键盘|鼠标|显示器|项目|device|project))?)/gi)).map((match) => match[1].trim());
    const englishPatternRefs = Array.from(normalizeLexiconText(text).matchAll(/((?:the new one|new one|this one|the current one|the previous one|previous one|the old one|old one|that one|this project|that project|the new project|new project|the previous project|previous project|old project|the previous headset|previous headset|the new headset|new headset|the previous monitor|previous monitor|the new monitor|new monitor|the previous keyboard|previous keyboard|the new keyboard|new keyboard|the previous mouse|previous mouse|the new mouse|new mouse)(?:\s+(?:bluetooth|wireless|current|new|old))?(?:\s+(?:headset|monitor|keyboard|mouse|device|project|earbuds))?)/gi)).map((match) => match[1].trim());
    return Array.from(new Set([...lexiconRefs, ...patternRefs, ...englishPatternRefs]));
}
export function classifyIssueFamilies(text) {
    const lowered = text.toLowerCase();
    const families = Object.entries(ISSUE_FAMILY_LEXICON)
        .filter(([, descriptor]) => includesLexiconTerm(lowered, descriptor.keywords))
        .map(([family]) => family);
    return families.length > 0 ? families : [];
}
export function detectIssueQualifier(text) {
    const lowered = text.toLowerCase();
    if (includesLexiconTerm(lowered, ISSUE_QUALIFIER_LEXICON.left))
        return 'left';
    if (includesLexiconTerm(lowered, ISSUE_QUALIFIER_LEXICON.right))
        return 'right';
    return 'generic';
}
export function inferIssueValue(text) {
    const families = classifyIssueFamilies(text);
    if (families.includes('connectivity_issue'))
        return '断连';
    if (families.includes('sound_issue'))
        return '杂音';
    if (families.includes('performance_issue'))
        return '卡顿';
    return '问题';
}
export function hasIssueSignal(text) {
    return classifyIssueFamilies(text).length > 0;
}
export function isStrongNewInstanceSignal(text) {
    return includesLexiconTerm(normalizeLexiconText(text), ENTITY_INSTANCE_SIGNAL_LEXICON.strongNew);
}
export function isStrongUpdateInstanceSignal(text) {
    return includesLexiconTerm(normalizeLexiconText(text), ENTITY_INSTANCE_SIGNAL_LEXICON.strongUpdate);
}
export function matchBindingReplyIntent(text) {
...
export function isBindFirstShortReply(text) {
    return matchBindingReplyIntent(text) !== null;
}
export function detectPendingBindingPromptType(text) {
    const normalized = normalizeLexiconText(text);
    if (PENDING_BINDING_PROMPT_PATTERNS.action.some((pattern) => pattern.test(normalized)))
        return 'action';
    ...
    if (hasEntityPromptPattern && hasRelativeReferences)
        return 'entity';
    if (/\?$|？$/.test(normalized))
        return 'question';
    return null;
}
export function hasLongTermMemorySignal(text) {
    const normalized = normalizeLexiconText(text);
    return Boolean(inferEntityTypeFromText(normalized)
...
export function extractIssueHints(text) {
    const normalized = normalizeLexiconText(text);
...
export function extractLatestIssueReference(text) {
    const normalized = normalizeLexiconText(text);
    const segments = normalized
        .split(/[，。,.!?！？]/)
        .map((segment) => segment.trim())
...
export function extractIssueRankingTokensFromText(text) {
    const normalized = normalizeLexiconText(text);
    const tokens = [];
    const qualifier = detectIssueQualifier(normalized);
    if (qualifier === 'left')
        tokens.push('left-ear');
    if (qualifier === 'right')
        tokens.push('right-ear');
    for (const family of classifyIssueFamilies(normalized)) {
        if (family === 'connectivity_issue')
            tokens.push('disconnect');
        if (family === 'sound_issue')
            tokens.push('noise');
        if (family === 'performance_issue')
            tokens.push('performance');
    }
    return Array.from(new Set(tokens));
}
export function extractDeviceCandidate(text) {
    const normalized = normalizeLexiconText(text);
...
export function extractDeviceAliasCandidates(text) {
    const normalized = normalizeLexiconText(text);
    ...
    if (nounMatch?.[1])
        aliases.add(nounMatch[1].replace(/s$/i, ''));
    if (/\bbluetooth\b/i.test(candidate))
        aliases.add('bluetooth');
    if (/\bwireless\b/i.test(candidate))
        aliases.add('wireless');
    return Array.from(aliases);
}
export function isOwnershipSignal(text) {
    return includesLexiconTerm(text, RELATION_PHRASE_MAP.owns);
}
export function extractOwnershipSignals(text) {
    const normalized = normalizeLexiconText(text);
...
export function extractProjectLinks(text) {
    const normalized = normalizeLexiconText(text);
...
export function extractProjectAliasCandidates(text) {
    const normalized = normalizeLexiconText(text);
    const aliases = new Set();
    ...
    for (const link of extractProjectLinks(normalized)) {
        aliases.add(link);
        if (link.includes('-'))
            aliases.add(link.replace(/-/g, ' '));
        if (/\s+project$/i.test(link))
            aliases.add(link.replace(/\s+project$/i, ''));
        if (/\s+sdk$/i.test(link) && /\bbluetooth\b/i.test(link))
            aliases.add('bluetooth sdk');
    }
    if (projectCandidate && /\bsdk\b/i.test(projectCandidate)) {
        if (/\bbluetooth\b/i.test(projectCandidate))
            aliases.add('bluetooth sdk');
        if (/\bbluetooth low energy\b/i.test(projectCandidate))
            aliases.add('ble sdk');
        if (/\biot\b/i.test(normalized))
            aliases.add('iot sdk');
    }
    for (const reference of extractRelativeReferences(normalized)) {
        if (/project|项目/i.test(reference))
            aliases.add(reference);
    }
    return Array.from(aliases).filter(Boolean);
}
export function extractConditionHints(text) {
    const normalized = normalizeLexiconText(text);
    const matches = normalized.match(/如果[^，。,.!?]+|if\s+[^,.!?]+/gi) || [];
    return Array.from(new Set(matches.map((match) => match.trim()).filter(Boolean)));
}
export function extractTemporalHints(text) {
    const normalized = normalizeLexiconText(text);
    const hints = [];
    if (/(今天|today)/i.test(normalized))
        hints.push('today');
    if (/(本周|this week)/i.test(normalized))
        hints.push('this_week');
    if (/(这个月|this month)/i.test(normalized))
        hints.push('this_month');
    if (/(近一年|past year|last year)/i.test(normalized))
        hints.push('past_year');
    if (/(半年|half year|six months)/i.test(normalized))
        hints.push('around_half_year_ago');
    return Array.from(new Set(hints));
}
export function extractPreference(text) {
    const matches = Array.from(text.matchAll(/(?<verb>喜欢|I like|讨厌|不喜欢|I dislike)\s*(?<target>[\u4e00-\u9fa5A-Za-z0-9._-]+(?:\s+[\u4e00-\u9fa5A-Za-z0-9._-]+){0,2})/gi));
    const last = matches[matches.length - 1];
    if (!last?.groups?.verb || !last.groups.target)
        return null;
    return {
...
export function extractNegativePreferenceCue(text) {
    const normalized = normalizeLexiconText(text);
...
export function extractExplicitNamedEntityCandidate(text) {
    const normalized = normalizeLexiconText(text);
    const explicitNamedDevice = normalized.match(/(?:i\s+bought(?:\s+(?:a|an|another))?(?:\s+new)?|i\s+have(?:\s+(?:a|an))?|i\s+had(?:\s+(?:a|an))?|i\s+got(?:\s+(?:a|an))?(?:\s+new)?|my|买了(?:一个|一副)?|我买了|我有一个|我有个|我的)\s+([A-Za-z][A-Za-z0-9._-]*(?:\s+[A-Za-z0-9._-]+){0,3})(?=\s+(?:for|in|during|on|yesterday|today|earlier|later|project|headset|monitor|keyboard|mouse|earbuds?)\b|[,.!?，。！？]|$)/i);
    ...
    if (!candidate)
        return null;
    if (/^(i like|i dislike)$/i.test(candidate))
        return null;
    return collapseSurfaceWhitespace(candidate);
}
export function extractApprovedArchiveProject(text) {
    const normalized = normalizeLexiconText(text);
...
export function inferReferenceType(reference, query) {
    const candidate = `${reference} ${query}`.toLowerCase();
    if (includesLexiconTerm(candidate, ENTITY_TYPE_LEXICON.device))
        return 'device';
    if (includesLexiconTerm(candidate, ENTITY_TYPE_LEXICON.project))
        return 'project';
    return undefined;
}
export function isLatestReference(reference) {
...
export function isPreviousReference(reference) {
    const normalized = reference.toLowerCase();
