...
export class ProjectionObservabilityStore {
    ...
    `).run(sampleId, input.projectionType, input.projectionName, input.checkpointStatus, input.pendingEvents, input.materializedCount, input.metadata ? JSON.stringify(input.metadata) : null, sampledAt);
    }
    ...
    compactOlderThan(cutoffTime, bucketMs = 60 * 60 * 1000) {
    ...
    `).all(safeBucketMs, safeBucketMs, cutoffTime);
    ...
    `);
        for (const row of rows) {
            ...
            insertStmt.run(rollupId, row.projection_type, row.projection_name, row.checkpoint_status, Math.round(row.avg_pending_events), Math.round(row.avg_materialized_count), safeBucketMs, row.sample_count, JSON.stringify({ storageTier: 'rollup' }), row.bucket_start);
        }
    ...
    compactRollupsOlderThan(cutoffTime, sourceBucketMs, targetBucketMs) {
      ...
      GROUP BY projection_type, projection_name, checkpoint_status, bucket_start
      ORDER BY bucket_start ASC
    `).all(targetBucketMs, targetBucketMs, cutoffTime, sourceBucketMs);
        const insertStmt = this.db.prepare(`
      INSERT OR REPLACE INTO projection_observability_rollups (
        rollup_id, projection_type, projection_name, checkpoint_status,
        pending_events, materialized_count, bucket_ms, sample_count, metadata_json, sampled_at
    ...
    `);
        for (const row of rows) {
            ...
            insertStmt.run(rollupId, row.projection_type, row.projection_name, row.checkpoint_status, Math.round(row.avg_pending_events), Math.round(row.avg_materialized_count), targetBucketMs, row.sample_count, JSON.stringify({ storageTier: 'rollup', sourceBucketMs }), row.bucket_start);
        }
    ...
    getHistoryPage(page = 1, pageSize = 20, filters) {
      ...
      `).get(...sourceParams);
        ...
        LIMIT ? OFFSET ?
      `).all(aggregateMode, ...sourceParams, ...sourceParams, safePageSize, offset);
        }
        else {
            const sourceSql = includeRollups
                ? `
      ...
      `).get(...sourceParams);
        ...
        ORDER BY sampled_at DESC, projection_name DESC
        LIMIT ? OFFSET ?
      `).all(...sourceParams, safePageSize, offset);
        }
        return {
            page: safePage,
