import { extractRelativeReferences, inferReferenceType, isLatestReference, isPreviousReference, normalizeLexiconText } from '../lexicon/coreMemoryLexicon.js';
export class QueryTimePendingEntityResolver {
    entityStore;
    getNeuronById;
    resolveBeforeTime;
    constructor(entityStore, getNeuronById, resolveBeforeTime) {
        this.entityStore = entityStore;
        this.getNeuronById = getNeuronById;
        this.resolveBeforeTime = resolveBeforeTime;
    }
    resolve(input) {
        ...
        return {
            ...
            resolvedEntityIds: Array.from(new Set(results
    ...
    collectRelativeReferences(input) {
    ...
    findMatchingPendingRecords(reference, pendingRecords, input) {
        const normalizedReference = normalizePendingReference(reference);
        const referenceSignals = new Set(extractRelativeReferences(normalizedReference).map(normalizePendingReference));
        if (referenceSignals.size === 0)
            referenceSignals.add(normalizedReference);
        const matchingPending = pendingRecords.filter((record) => {
            ...
            if (pendingSignals.size === 0)
                pendingSignals.add(normalizedPending);
            const sharesRelativePolarity = Array.from(referenceSignals).some((signal) => Array.from(pendingSignals).some((pendingSignal) => (isPreviousReference(signal) && isPreviousReference(pendingSignal))
        ...
        if (projectScopedPending.length > 0)
            return projectScopedPending;
        return this.findSameTypePendingFallback(reference, pendingRecords, input);
    }
    canResolveNarrowly(topCandidate, secondCandidate) {
        if (!topCandidate)
            return false;
    ...
    inferOverallStatus(results) {
        if (results.some((result) => result.queryTimeResolutionStatus === 'resolved_narrowly'))
            return 'resolved_narrowly';
        if (results.some((result) => result.queryTimeResolutionStatus === 'narrowed_but_still_ambiguous'))
            return 'narrowed_but_still_ambiguous';
        if (results.some((result) => result.queryTimeResolutionStatus === 'unresolved_explicit'))
            return 'unresolved_explicit';
        if (results.some((result) => result.queryTimeResolutionStatus === 'not_needed'))
            return 'not_needed';
        return 'not_applicable';
    }
    inferOverallReason(results) {
        const priority = [
        ...
        for (const reason of priority) {
            if (results.some((result) => result.queryTimeResolutionReason === reason))
                return reason;
        }
        return 'no_relative_pending_reference_in_query';
    }
    inferNoPendingReason(reference, pendingRecords, input) {
        if (this.hasWriteTimeSufficientEntityResolution(reference, input)) {
            return 'write_time_resolution_already_sufficient';
        }
        if (pendingRecords.length === 0)
            return 'pending_capture_missing_or_too_weak';
        return 'no_matching_pending_reference';
    }
    hasWriteTimeSufficientEntityResolution(reference, input) {
        const normalizedReference = normalizePendingReference(reference);
        return input.semanticCompilation.relativeReferences.some((item) => normalizePendingReference(item) === normalizedReference)
            && input.baseEntityResolution.disambiguation.some((item) => normalizePendingReference(item.reference) === normalizedReference
                && item.candidates.length > 0);
    }
    computePendingCaptureMetrics(pendingRecords, results) {
    ...
    inferPendingScopedProjectId(records) {
    ...
    findSameTypePendingFallback(reference, pendingRecords, input) {
        const normalizedReference = normalizePendingReference(reference);
        const inferredType = inferReferenceType(normalizedReference, input.query);
        if (!inferredType || hasDirectionalRelativeSignal(normalizedReference))
            return [];
        const sameTypePending = pendingRecords.filter((record) => {
            if (record.entityType && record.entityType !== inferredType)
                return false;
            if (!input.projectId || !record.contextNeuronId)
                return true;
            const neuron = this.getNeuronById(record.contextNeuronId);
            return !neuron?.metadata.projectId || neuron.metadata.projectId === input.projectId;
        });
...
function emptyPendingCaptureMetrics() {
    return {
...
function normalizePendingReference(reference) {
    return normalizeLexiconText(reference).toLowerCase();
}
function hasDirectionalRelativeSignal(reference) {
