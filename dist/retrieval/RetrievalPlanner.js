export class RetrievalPlanner {
    plan(ir) {
        ...
        if (ir.semantics.guardHints.length > 0
    ...
    inferIntent(ir, normalizedQuery) {
        if (ir.intentType === 'trace')
            return 'trace';
        if (/(喜欢|讨厌|偏好|prefer|preference|dislike)/i.test(normalizedQuery))
            return 'preference_lookup';
        if (/(决定|选择|choose|decision|decide)/i.test(normalizedQuery))
            return 'decision_lookup';
        if (/(约束|限制|必须|不能|constraint|rule)/i.test(normalizedQuery))
            return 'constraint_lookup';
        if (/(事实|配置|地址|api|endpoint|what is|which)/i.test(normalizedQuery))
            return 'fact_lookup';
        if (/(debug|bug|error|exception|报错)/i.test(normalizedQuery))
