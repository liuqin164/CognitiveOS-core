...
export class NeuronContextTool {
    memoryGraph;
    graphEdgeStore;
    constructor(memoryGraph, graphEdgeStore) {
        this.memoryGraph = memoryGraph;
        this.graphEdgeStore = graphEdgeStore;
    }
    execute(neuronId, projectId) {
        ...
        if (this.graphEdgeStore?.list) {
            ...
            for (const edge of activeEdges) {
                if (edge.fromNodeId === neuronId && edge.toNodeId !== undefined)
                    neighborIds.add(edge.toNodeId);
                if (edge.toNodeId === neuronId && edge.fromNodeId !== undefined)
