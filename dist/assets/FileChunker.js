...
export class FileChunker {
    targetTokenEstimate;
    constructor(targetTokenEstimate = 420) {
        this.targetTokenEstimate = targetTokenEstimate;
    }
    chunk(blocks) {
        const chunks = [];
        let current = [];
        ...
        blocks.forEach((block, index) => {
            ...
            if (forceOwnChunk && current.length > 0)
                flush(index - 1);
            if (current.length === 0)
                currentStart = index;
            current.push(block);
            tokens += blockTokens;
            if (forceOwnChunk || tokens >= this.targetTokenEstimate)
                flush(index);
        });
