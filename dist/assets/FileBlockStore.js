...
export class FileBlockStore {
    ...
    `);
        return blocks.map((block, index) => {
            ...
            stmt.run(record.blockId, record.assetId, record.blockIndex, record.kind, record.text, record.page || null, record.sheetName || null, record.rowStart || null, record.rowEnd || null, record.columnStart || null, record.columnEnd || null, record.lineStart || null, record.lineEnd || null, record.startMs || null, record.endMs || null, record.selector || null, record.symbolName || null, record.metadata ? JSON.stringify(record.metadata) : null, record.createdAt);
            return record;
        });
    }
    listByAsset(assetId) {
    ...
    listRange(assetId, startIndex, endIndex) {
        return this.db.prepare(`
      SELECT * FROM file_blocks
      WHERE asset_id = ? AND block_index BETWEEN ? AND ?
      ORDER BY block_index ASC
    `).all(assetId, startIndex, endIndex).map((row) => this.mapRow(row));
    }
    mapRow(row) {
        return {
