...
import { parseNormalizationMetadata } from '../utils/ConversationMarkdownNormalization.js';
...
export function classifyLane(source) {
    ...
    if (source.adapterKind === 'openclaw_persona'
...
function hasContractMismatch(diagnostics) {
...
function buildAlternateSource(source, adapterKind) {
    return {
        ...source,
        adapterKind
    };
}
function safeAdapt(source, window, applyWindow) {
    ...
    return adapter.adapt(source, snapshot, applyWindow && !source.tags?.includes('ingest:profile_only')
        ? { start: window.start, end: window.end }
        : undefined);
}
function looksLikeNormalizationOnlyPath(sourcePath) {
    const ext = extname(sourcePath).toLowerCase();
    return ext === '.jsonl' || ext === '.json' || ext === '.csv' || ext === '.tsv' || ext === '.log' || ext === '.txt';
}
function suggestedNormalizerForPath(sourcePath) {
    const ext = extname(sourcePath).toLowerCase();
    if (ext === '.jsonl')
        return 'bun run normalize:jsonl -- --input ... --output ...';
    if (ext === '.json')
        return 'bun run normalize:json-array -- --input ... --output ...';
    if (ext === '.csv' || ext === '.tsv')
        return 'bun run normalize:csv -- --input ... --output ...';
    if (ext === '.log' || ext === '.txt')
        return 'export/preprocess outside the brain, then run a custom normalizer script';
    return undefined;
}
function looksLikeAppPrivateJsonExport(content, sourcePath) {
...
function describeNormalizationFamily(family) {
    switch (family) {
...
function probeReroute(source, window, snapshotContent) {
...
export function inspectSources(sources, window, seenHashesBySourceId = new Map()) {
    const sourceReports = sources.map((source) => {
        ...
        const seenHashes = seenHashesBySourceId.get(source.sourceId) || new Set();
        ...
        if (normalizationMetadata.isNormalized && source.adapterKind === 'conversation_markdown' && !mismatch) {
            const normalizedFamilyLabel = normalizationMetadata.originalInputFamily
                ...
                : 'normalized export';
            recommendations.push(`Normalized ${normalizedFamilyLabel} detected. Recommended onboarding path: export/preprocess -> normalize -> batch:preflight -> batch:consolidate --strict -> recall.`);
        }
...
export function evaluateStrictPreflightFailures(sources, preflight) {
    const failures = [];
    for (const source of sources) {
        if (!isCriticalSource(source))
            continue;
...
export function evaluateStrictPostIngestFailures(sources, preflight, sourceResults) {
    const failures = [];
    for (const source of sources) {
        if (!isCriticalSource(source))
            continue;
        ...
        if (!report || !result)
            continue;
        const misroutingDiagnostic = result.diagnostics.some((item) => item.code === 'conversation_contract_mismatch' || item.code === 'soul_contract_mismatch');
