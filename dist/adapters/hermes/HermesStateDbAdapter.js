...
export class HermesStateDbAdapter {
    kind = 'hermes_state_db';
    adapterVersion = 'hermes-state-db-v1';
    adapt(source, snapshot, window) {
        ...
        try {
            ...
            const rows = db.prepare(`SELECT rowid AS __rowid, * FROM ${quoteIdentifier('messages')}`).all();
            const mapped = rows
            ...
            diagnostics.push(this.diagnostic(source, 'error', 'hermes_state_db_read_failed', `Failed to read Hermes state database: ${error instanceof Error ? error.message : String(error)}`));
        }
        finally {
            db?.close();
        }
    ...
    toRecord(source, snapshot, row, columns, index, diagnostics) {
        ...
        if (timeSource.kind === 'insert_time_fallback') {
            diagnostics.push(this.diagnostic(source, 'warning', 'hermes_state_db_insert_time_fallback', `Message ${messageId} did not expose occurredAt/timestamp; using InsertTime/import time fallback.`));
        }
        return {
    ...
    result(source, snapshot, records, diagnostics) {
        return {
            source,
            snapshot: {
    ...
    diagnostic(source, severity, code, message) {
        return {
            severity,
            code,
            message,
...
function normalizeRole(value) {
    ...
    if (['assistant', 'agent', 'ai', 'bot', 'model'].includes(normalized))
        return 'agent';
    if (['system', 'sys', 'tool'].includes(normalized))
        return 'system';
    if (['narrator', 'note', 'memo'].includes(normalized))
        return 'narrator';
    return 'user';
}
function pickTimestampSource(row) {
    const lowerMap = new Map(Object.keys(row).map((key) => [key.toLowerCase(), key]));
    for (const column of MESSAGE_TIME_COLUMNS) {
        ...
        if (value)
            return { value, column: actual, kind: 'message_time' };
    }
    for (const column of INSERT_TIME_COLUMNS) {
        ...
        const value = textFromValue(row[actual]);
        if (value)
            return { value, column: actual, kind: 'insert_time_fallback' };
    }
    return { kind: 'missing' };
}
function parseTimestampValue(value, fallback) {
    if (!value)
        return fallback;
    const trimmed = value.trim();
    if (/^\d+(?:\.\d+)?$/.test(trimmed)) {
