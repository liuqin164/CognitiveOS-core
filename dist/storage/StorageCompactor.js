...
export function compactStorage(input) {
        ...
        if (!input.dryRun && eligible.length > 0) {
            ...
            const write = db.transaction((rows) => {
                for (const row of rows)
                    deleteOne.run(row.neuron_id, input.dimension ?? null, input.dimension ?? null);
            });
            write(eligible);
        }
...
function getRawEventCount(db) {
    const row = db.prepare(`SELECT COUNT(*) AS count FROM memory_events`).get();
    return row?.count || 0;
}
function getVectorStats(db, input) {
    const conditions = [];
    const params = [];
...
function listEligibleVectors(db, input, statuses) {
