...
export const migration_0004 = {
    ...
    `);
        ...
        const addColumn = (col, definition) => {
            const exists = db.prepare(`SELECT COUNT(*) AS n FROM pragma_table_info('meta_proposals') WHERE name = ?`).get(col).n > 0;
            if (!exists) {
    ...
    down(db) {
        ...
        const existingColumns = new Set(db.prepare(`PRAGMA table_info(meta_proposals)`).all()
